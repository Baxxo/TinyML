...
let apply_subst_to_scheme (Forall(tvs, t)) (s: subst): scheme =
    ...
    let subst_prime = List.filter (fun (tv, _) -> not (Set.contains tv tvs)) s
    Forall(tvs, apply_subst t subst_prime)
...
let rec apply_subst_to_env (env: scheme env) (s: subst): scheme env =
    ...
    List.map (fun (id, scheme) -> (id, apply_subst_to_scheme scheme s)) env
...
let generalize env t =
    ...
    #endif
    Forall(diff, t)*)
    let alpha = freevars_ty t - freevars_scheme_env env
    Forall(alpha, t)
    ...
    let nr =
        ...
        | Tuple tup ->
            let infer_expr (accu_t, accu_s) expr =
                ...
                (accu_t @ [apply_subst t_i s_i]), (s_i @ accu_s)
